...
```
NAME                            GPU   CPU      MEM
gpu-node-1   8     79500m   ~1.2Ti
gpu-node-2   8     79500m   ~1.2Ti
```
- 16 GPUs total
...
|---|---|---|
| `${BLOCK_STORAGE_CLASS}` | `block.csi.driver` | ✅ Yes |
| `${BLOCK_STORAGE_CLASS_STD}` | `block.csi.driver` | No |
| `nfs-csi` | `nfs.csi.k8s.io` | No |

> Use `nfs-csi` for shared ReadWriteMany volumes (MinIO, Feast offline store). Use `${BLOCK_STORAGE_CLASS}` for block storage (Redis, Milvus).
...
- [ ] Fix or request the cluster admin to fix
...
- [ ] Set resource quotas (coordinate with the cluster admin to avoid conflicting with other teams)
...
- **Options:**
  - A) Request the cluster admin install Slinky operator (preferred — required for Slurm demo story)
...
- [ ] Confirm with the cluster admin if Slurm/Slinky can be installed
...
- [ ] Deploy MinIO in `smartshop` namespace (or request shared S3 endpoint from the cluster admin)
...
- Storage: `redis-data` PVC, 10Gi, `${BLOCK_STORAGE_CLASS}` (RWO)
